...
if (constrain_par == 1) {
  if (constraint_type == 1) {
    if (hier_type == 1 || hier_type == 4 || hier_type == 5 || hier_type == 7) {
      restrict_low[1] ~normal(legis_pred[1, (num_legis - num_constrain_l + 1):num_legis, ] * legis_x_cons, 10);
      ...
      if (T > 1) {
        for (t in 2:T) {
          restrict_low[t] ~normal(restrict_low[t - 1] +
            legis_pred[t, (num_legis - num_constrain_l + 1):num_legis, ] * legis_x_cons, 10);
      ...
      if (hier_type == 1) {
        ...
        for (b in 1:rows(sigma_abs_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_abs_free[b] | 1, abs_discrim_sd), normal_lpdf(sigma_abs_free[b] | -1, abs_discrim_sd));
        }
        for (b in 1:rows(sigma_reg_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_reg_free[b] | 1, reg_discrim_sd), normal_lpdf(sigma_reg_free[b] | -1, reg_discrim_sd));
        }
      } else if (hier_type == 4) {
      ...
      restrict_low[1] ~normal(0, 10);
      L_free[1] ~normal(0, legis_sd);
      //add basic integrated time-series prior
      if (T > 1) {
        for (t in 2:T) {
          restrict_low[t] ~normal(restrict_low[t - 1], 10);
    ...
    if (hier_type == 1 || hier_type == 4 || hier_type == 5 || hier_type == 7) {
      ...
      restrict_high[1] ~normal(legis_pred[1, (num_legis - num_constrain_l + 1):num_legis, ] * legis_x_cons, 10);
      ...
      //legis the same for all other hier_type combinations
      restrict_high[1] ~normal(0, 10);
      L_free[1] ~normal(0, legis_sd);
      //add basic integrated time-series prior
      if (T > 1) {
        for (t in 2:T) {
          restrict_high[t] ~normal(restrict_high[t - 1], 10);
          L_free[t] ~normal(L_free[t - 1], legis_sd);
        }
      }
    ...
    if (hier_type == 1 || hier_type == 4 || hier_type == 5 || hier_type == 7) {
      ...
      restrict_high[1] ~normal(legis_pred[1, (num_legis - (2 * num_constrain_l) + 1):(num_legis - num_constrain_l), ] * legis_x_cons, 10);
      restrict_low[1] ~normal(legis_pred[1, (num_legis - num_constrain_l + 1):num_legis, ] * legis_x_cons, 10);
      L_free[1] ~normal(legis_pred[1, 1:(num_legis - num_constrain_l), ] * legis_x, legis_sd);
      //add basic integrated time-series prior
      if (T > 1) {
        for (t in 2:T) {
          restrict_high[t] ~normal(restrict_high[t - 1] +
            legis_pred[t, (num_legis - (2 * num_constrain_l) + 1):(num_legis - num_constrain_l), ] * legis_x_cons, 10);

          restrict_low[t] ~normal(restrict_high[t - 1] +
            legis_pred[t,
              (num_legis - num_constrain_l + 1):num_legis, ] * legis_x_cons, 10);
  ...
  if (constraint_type == 1) {
    if (hier_type == 3 || hier_type == 5 || hier_type == 6 || hier_type == 7) {
      ...
      restrict_low[1] ~normal(sax_pred[(num_bills - num_constrain_sa + 1):num_legis, ] * sigma_abs_x_cons, 10);
      if (hier_type == 3) {
        for (b in 1:rows(sigma_reg_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_reg_free[b] | 1, reg_discrim_sd), normal_lpdf(sigma_reg_free[b] | -1, reg_discrim_sd));
        }
    ...
    if (hier_type == 3 || hier_type == 5 || hier_type == 6 || hier_type == 7) {
      //all combinations where hier_type parameters in absences
      sigma_abs_free~normal(sax_pred[num_bills - num_constrain_sa, ] * sigma_abs_x, 10);
      restrict_high[1] ~normal(sax_pred[(num_bills - num_constrain_sa + 1):num_bills, ] * sigma_abs_x_cons, 10);
      if (hier_type == 3) {
        for (b in 1:rows(sigma_reg_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_reg_free[b] | 1, reg_discrim_sd), normal_lpdf(sigma_reg_free[b] | -1, reg_discrim_sd));
        }
    ...
    if (hier_type == 3 || hier_type == 5 || hier_type == 6 || hier_type == 7) {
      //all combinations where hier_type parameters in absences
      sigma_abs_free~normal(sax_pred[num_bills - num_constrain_sa, ] * sigma_abs_x, 10);
      restrict_high[1] ~normal(sax_pred[(num_bills - (2 * num_constrain_sa) + 1):(num_bills - num_constrain_sa), ] * sigma_abs_x_cons, 10);
      restrict_low[1] ~normal(sax_pred[(num_bills - num_constrain_sa + 1):num_bills, ] * sigma_abs_x_cons, 10);
      if (hier_type == 3) {
        for (b in 1:rows(sigma_reg_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_reg_free[b] | 1, reg_discrim_sd), normal_lpdf(sigma_reg_free[b] | -1, reg_discrim_sd));
        }
  ...
  if (constraint_type == 1) {
    if (hier_type == 3 || hier_type == 5 || hier_type == 6 || hier_type == 7) {
      ...
      restrict_high[1] ~normal(srx_pred[(num_bills - num_constrain_sr + 1):num_bills, ] * sigma_reg_x_cons, 10);
      if (hier_type == 2) {
        for (b in 1:rows(sigma_abs_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_abs_free[b] | 1, abs_discrim_sd), normal_lpdf(sigma_abs_free[b] | -1, abs_discrim_sd));
        }
    ...
    if (hier_type == 3 || hier_type == 5 || hier_type == 6 || hier_type == 7) {
      //all combinations where hier_type parameters in absences
      sigma_reg_free~normal(srx_pred[num_bills - num_constrain_sr, ] * sigma_reg_x, 10);
      restrict_high[1] ~normal(srx_pred[(num_bills - (2 * num_constrain_sr) + 1):(num_bills - num_constrain_sr), ] * sigma_reg_x_cons, 10);
      restrict_low[1] ~normal(srx_pred[(num_bills - num_constrain_sr + 1):num_bills, ] * sigma_reg_x_cons, 10);
      if (hier_type == 2) {
        for (b in 1:rows(sigma_abs_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_abs_free[b] | 1, abs_discrim_sd), normal_lpdf(sigma_abs_free[b] | -1, abs_discrim_sd));
        }
    ...
    if (hier_type == 3 || hier_type == 5 || hier_type == 6 || hier_type == 7) {
      ...
      if (hier_type == 2) {
        for (b in 1:rows(sigma_abs_free)) {
          target += log_mix(0.5, normal_lpdf(sigma_abs_free[b] | 1, abs_discrim_sd), 
