...
.make_stan_data <- function(idealdata=NULL,...) {
  ...
  if(vary_ideal_pts==5) {
    ...
    old_bounds <- c(min(time_ind,na.rm=TRUE),max(time_ind, na.rm=TRUE))
...
process_init_draws <- function(init, num_procs, model_variables = NULL,
  ...
  variable_names <- variables(init, with_indices = FALSE)
...
.normalize <- function(outcome,true_bounds=NULL) {
  ...
  if(is.na(min(outcome, na.rm=TRUE)) || is.infinite(min(outcome, na.rm=TRUE))) {
  ...
  if(!is.null(true_bounds)) {
    ...
    min_out <- min(outcome, na.rm=TRUE)
    max_out <- max(outcome, na.rm=TRUE)
...
.prepare_legis_data <- function(object,
  ...
  if(length(unique(object@score_data@score_matrix$time_id))>1 && type!='variance') {
    ...
    person_ids <- tibble(long_name=person_params$legis) %>% 
      distinct
    legis_nums <- stringr::str_extract_all(person_ids$long_name,'[0-9]+',simplify=TRUE)
  ...
  if('matrix' %in% class(input_matrix)) {
    ...
    to_array <- lapply(split(to_spread,pull(to_spread,!!third_dim_var)), function(this_data) {
      ...
      if('try-error' %in% class(spread_it)) {
        message('Failed to find unique covariate values for dataset:')
        message(paste(utils::capture.output(this_data), collapse = "\n"))
        stop()
      }
      spread_it <- spread_it %>% 
...
.init_stan <- function(chain_id=NULL,
  ...
  generate_initial_values <- function(num_bills, num_legis, 
   ...
   out_list <-  list(
      ...
      ordbeta_cut = if(num_ordbeta>0) matrix(rep(c(-1,1),times=num_ordbeta),ncol=2,nrow=num_ordbeta,byrow=TRUE) else array(numeric(0),dim=c(1,0)),
...
.calc_true_pts <- function(obj) {
  ...
  # Reconstruct [draws, time, persons] array from flat draws matrix
  over_time_mat <- posterior::as_draws_matrix(obj@stan_samples$draws('L_tp1'))
  cn <- colnames(over_time_mat)
  t_idx <- as.integer(sub("L_tp1\\[([0-9]+),.*", "\\1", cn))
  p_idx <- as.integer(sub(".*,([0-9]+)\\]", "\\1", cn))
  n_draws <- nrow(over_time_mat)
  over_time <- array(NA_real_, dim = c(n_draws, max(t_idx), max(p_idx)))
  for(j in seq_along(cn)) over_time[, t_idx[j], p_idx[j]] <- over_time_mat[, j]

  drift <- posterior::as_draws_matrix(obj@stan_samples$draws('L_full'))
...
.extract_nonp <- function(object, pars, permuted = TRUE, 
            ...
            if (object@mode == 1L) {
              message("Stan model '", object@model_name, "' is of mode 'test_grad'; ",
                  "sampling is not conducted.")
              return(invisible(NULL))
            } else if (object@mode == 2L) {
              message("Stan model '", object@model_name, "' does not contain samples.")
              return(invisible(NULL))
            }
...
.get_cuts_cov <- function(k=NULL,
  ...
  .draws_col <- function(par) {
    as.matrix(posterior::as_draws_matrix(obj@stan_samples$draws(par)))
  }

  if(m %in% c(3,4)) {

    if(k==1) {
      cutp <- .draws_col(paste0("steps_votes",K,"[",k,"]"))
    } else if(k>1 && k<K) {
      cutp <- cbind(.draws_col(paste0("steps_votes",K,"[",k-1,"]")),
                    .draws_col(paste0("steps_votes",K,"[",k,"]")))
    } else {
      cutp <- .draws_col(paste0("steps_votes",K,"[",k-1,"]"))
    }
    ...
    if(k==1) {
      cutp <- .draws_col(paste0("grm_steps_votes",K,"[",item_num,",",k,"]"))
    } else if(k>1 && k<K) {
      cutp <- cbind(.draws_col(paste0("steps_votes",K,"[",item_num,",",k-1,"]")),
                    .draws_col(paste0("steps_votes",K,"[",item_num,",",k,"]")))
    } else {
      cutp <- .draws_col(paste0("steps_votes",K,"[",item_num,",",k-1,"]"))
    }
  }
...
.add_person_cov <- function(all_time, 
  ...
  if(!is.null(legis_x) && sum(c(legis_x))!=0) {
    ...
    message("Adding in hierarchical covariates values to the time-varying person scores.")
    ...
    message("Collapsing covariates to person and time IDs.")
